...
import {
  ...
  ScanLine, SlidersHorizontal, Radio, History, ClipboardCheck,
} from "lucide-react";
...
function FacilityOpsOverview() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.28fr_.72fr]">
      <AirportPanel title={vi ? "Bức tranh vận hành cơ sở vật chất sân bay" : "Airport facility operations picture"} subtitle={vi ? "EBO hợp nhất BMS, EMS, CCTV, ACS, Parking và các hệ kỹ thuật" : "EBO unifies BMS, EMS, CCTV, ACS, parking and technical systems"}>
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
      ...
      <AirportPanel title={vi ? "Sự kiện liên hệ thống" : "Cross-system events"}><div className="p-3"><AirportTimeline events={[
        ["11:42","BMS","AHU T1-L2 CO₂ above comfort threshold","Terminal T1","Warning"],
        ...
        ["11:24","ACS","Airside credential denied","Gate A-04","Info"],
        ["11:16","Parking","P2 occupancy reached 88%","Landside","Warning"],
      ]} /></div></AirportPanel>
    </div>
  </>;
}
  ...
  return <>
    <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">
      {[
        ...
        [vi ? "Điểm đang quét" : "Scanning points", "124,860", "6 sec"], [vi ? "Lịch vận hành" : "Active schedules", "486", "Airport-wide"],
...
function EnergyPowerOperations() {
  ...
  const sources = [
    ...
    { name: "Airfield Lighting", value: "1.4 MW", state: "Online" },
    { name: "BHS & Passenger Bridges", value: "2.1 MW", state: "Online" },
  ];
  return <>
    <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[
    ...
    <div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      <AirportPanel title={vi?"Sơ đồ điện một sợi sân bay":"Airport single-line power diagram"} action={<select value={source} onChange={e=>setSource(e.target.value)} className="airport-select"><option>Grid A</option><option>Grid B</option><option>Generator Bus</option><option>UPS Bus</option></select>}>
        <div className="relative min-h-[520px] overflow-hidden p-5" style={{backgroundImage:"radial-gradient(circle at 1px 1px,rgba(34,211,238,.12) 1px,transparent 0)",backgroundSize:"22px 22px"}}>
          <div className="grid h-full min-h-[470px] grid-cols-5 items-center gap-3">{[
            [source,"110 kV","normal"],["Transformer TR-01","40 MVA","normal"],["Main Bus MDB-A","22 kV","normal"],["Distribution","8 substations","normal"],["Airport Loads","12.8 MW","warning"],
...
function CctvVmsOperations() {
  const { language }=useAirportLanguage();
  const vi=language==="vi";
  const cameras = Array.from({length:12},(_,i)=>({id:`CAM-${String(i+1).padStart(3,"0")}`,zone:["Terminal","Airside","Perimeter","Cargo"][i%4],status:[3,9].includes(i)?"Degraded":"Live",event:[2,7].includes(i)?"Analytics event":"Normal",resolution:i%3===0?"4K":"1080p",retention:"30 days"}));
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      <AirportPanel title={vi?"Video wall VMS":"VMS video wall"}><div className="grid grid-cols-4 gap-2 p-3">{cameras.map((cam)=><button key={cam.id} onClick={()=>{setSelected(cam);setPlaying(true);}} className="relative aspect-video overflow-hidden rounded-lg border border-white/[.07] bg-[radial-gradient(circle_at_center,#17334b_0,#06111f_72%)]"><div className="absolute inset-0 opacity-30" style={{backgroundImage:"linear-gradient(rgba(34,211,238,.08) 1px,transparent 1px),linear-gradient(90deg,rgba(34,211,238,.08) 1px,transparent 1px)",backgroundSize:"20px 20px"}}/><Camera className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-cyan-300/40" size={30}/><div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-[#04111f]/85 px-2 py-1.5"><span className="text-[8px] text-white">{cam.id}</span><span className={`h-1.5 w-1.5 rounded-full ${cam.status==="Live"?"bg-emerald-300":"bg-amber-300"}`}/></div>{cam.event!=="Normal"&&<span className="absolute right-2 top-2 rounded bg-amber-400/15 px-1.5 py-1 text-[7px] text-amber-200">AI EVENT</span>}</button>)}</div></AirportPanel>
    ...
    <EboOperationalModal open={Boolean(selected)} onClose={()=>setSelected(null)} eyebrow="VMS integrated operational view" title={`${selected?.id ?? "Camera"} · ${selected?.zone ?? ""}`} subtitle="Video remains governed by the source VMS; EBO presents linked status, events and workflow." status={selected?.status==="Degraded"?"warning":"normal"} statusLabel={selected?.status ?? "Live"} fields={[
      ...
      <div className="grid gap-4 xl:grid-cols-[1.3fr_.7fr]"><div className="relative min-h-[520px] overflow-hidden rounded-xl border border-cyan-400/18 bg-[radial-gradient(circle_at_45%_40%,#163550_0,#06111f_65%)]"><div className="absolute inset-0 opacity-35" style={{backgroundImage:"linear-gradient(rgba(34,211,238,.09) 1px,transparent 1px),linear-gradient(90deg,rgba(34,211,238,.09) 1px,transparent 1px)",backgroundSize:"30px 30px"}}/><div className="absolute inset-x-[10%] bottom-[12%] top-[20%] rounded-[38px] border border-white/[.08] bg-[#0a1d31]/70"/><div className="absolute bottom-[18%] left-[14%] h-28 w-32 rounded-lg border border-cyan-300/20 bg-cyan-400/[.06]"/><div className="absolute bottom-[18%] right-[17%] h-40 w-20 rounded-lg border border-white/[.08] bg-white/[.03]"/><ScanLine className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-cyan-300/60 ${playing?"animate-pulse":""}`} size={54}/><div className="absolute left-3 top-3 flex items-center gap-2 rounded-lg bg-black/45 px-3 py-2 text-[9px] text-white"><span className={`h-2 w-2 rounded-full ${playing?"bg-red-400":"bg-slate-500"}`}/>{playing?"LIVE":"PAUSED"} · {selected?.id}</div><div className="absolute bottom-3 left-3 right-3 flex items-center justify-between rounded-xl border border-white/[.08] bg-black/45 px-3 py-2"><span className="text-[9px] text-slate-300">12:41:28 · UTC+7</span><span className="text-[9px] text-cyan-200">AI analytics overlay active</span></div></div><AirportPanel title="PTZ, analytics and event history"><div className="grid grid-cols-3 gap-2 p-3">{["Pan left","Tilt up","Zoom +","Home","Auto track","Privacy mask"].map(label=><button key={label} onClick={()=>toast.success(`${label} · demo command audited`)} className="rounded-lg border border-white/[.07] bg-white/[.03] p-3 text-[9px] text-slate-300 hover:border-cyan-400/20 hover:text-cyan-200">{label}</button>)}</div><div className="space-y-2 border-t border-white/[.06] p-3">{[["12:39:18","Motion analytics","Normal"],["12:31:04","Object classification","Person"],["12:16:42","Stream health","Recovered"],["11:58:11","Operator view","Opened"]].map(([time,event,state])=><div key={time} className="flex justify-between rounded-lg bg-white/[.03] p-3 text-[9px]"><span className="font-mono text-cyan-300">{time}</span><span className="text-slate-300">{event}</span><span className="text-emerald-300">{state}</span></div>)}</div></AirportPanel></div>
    </EboOperationalModal>
  </>;
}
...
function AccessControlOperations(){
  ...
  const vi=language==="vi";
  const rows=[
    {id:"ACS-T1-001",name:"Terminal T1 main entry",state:"Online",events:"12,840",denied:"0",zone:"Public / Landside",policy:"Open policy"},
    {id:"ACS-AIR-018",name:"Airside Gate A18",state:"Online",events:"4,286",denied:"2",zone:"Airside",policy:"Permit + role"},
    {id:"ACS-CARGO-04",name:"Cargo Gate 04",state:"Online",events:"2,148",denied:"5",zone:"Cargo & Customs",policy:"Restricted"},
...
function FireLifeSafetyOperations(){
  const {language}=useAirportLanguage(); const vi=language==="vi";
  const zones=Array.from({length:42},(_,i)=>({id:`FZ-${String(i+1).padStart(2,"0")}`,state:[11,28].includes(i)?"Supervisory":"Normal",area:["Terminal T1","Airside","Cargo","ATC"][i%4],devices:168+i*3}));
  const signals=[
    {id:"SD-T1-284",message:"Smoke detector review",state:"Warning",area:"Terminal T1"},{id:"SPK-CARGO-04",message:"Sprinkler pressure normal",state:"Normal",area:"Cargo"},{id:"VES-T1-02",message:"Voice evacuation ready",state:"Normal",area:"Terminal T1"},{id:"FACP-ATC-01",message:"Panel normal",state:"Normal",area:"ATC"},{id:"HYD-AIRSIDE-12",message:"Hydrant inspected",state:"Normal",area:"Airside"},{id:"STAIR-PRESS-08",message:"Fan test due",state:"Warning",area:"Terminal T1"},
  ];
...
function WaterUtilitiesOperations(){
  const {language}=useAirportLanguage(); const vi=language==="vi";
  const assets=[
    {name:"City supply",value:"4.2 bar",state:"Normal",type:"Supply"},{name:"Raw tank",value:"78%",state:"Normal",type:"Storage"},{name:"Treatment",value:"98.8%",state:"Normal",type:"Quality"},{name:"Airport ring",value:"3.8 bar",state:"Normal",type:"Distribution"},{name:"Terminal / Airside",value:"6,840 m³",state:"Normal",type:"Demand"},{name:"Leak Zone W-07",value:"3.4 L/min anomaly",state:"Warning",type:"Leak"},
  ];
...
function VerticalTransportOperations(){
  const {language}=useAirportLanguage(); const vi=language==="vi";
  const units=Array.from({length:12},(_,i)=>({id:`${i%2?"ESC":"EL"}-${String(i+1).padStart(2,"0")}`,zone:["T1 Departures","T1 Arrivals","Parking P2","Cargo"][i%4],state:[5,10].includes(i)?"Service due":"Running",trips:840+i*46,type:i%2?"Escalator":"Elevator",floor:i%2?"L1-L2":`L${i%4+1}`}));
...
const CROSS_DOMAIN_ALARMS = [
  ["EBO-260713-001","11:42","High","BMS","GBIA-AHU-T1-L2-01","CO₂ above comfort threshold","Acknowledged","Terminal FM"],
  ["EBO-260713-002","11:38","Medium","EMS","MDB-02","Demand peak forecast","Open","Energy Ops"],
  ["EBO-260713-003","11:31","Critical","CCTV","CAM-P07-04","Perimeter intrusion correlation","Investigating","Airport Security"],
  ["EBO-260713-004","11:24","Low","ACS","AIR-GATE-A04","Credential denied","Closed","Access Control"],
  ["EBO-260713-005","11:16","Medium","Parking","PARK-P2","Occupancy reached 88%","Open","Landside Ops"],
