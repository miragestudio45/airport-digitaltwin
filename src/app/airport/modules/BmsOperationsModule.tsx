...
const BMS_ALARMS = [
  { id: "BMS-ALM-2407", time: "10:42:18", priority: "High", asset: "GBIA-AHU-T1-L2-01", message: "CO₂ above comfort threshold", state: "Acknowledged", owner: "Terminal FM" },
  { id: "BMS-ALM-2406", time: "10:36:04", priority: "Medium", asset: "GBIA-VAV-T1-L3-05", message: "Airflow deviation > 12%", state: "Open", owner: "HVAC Ops" },
  { id: "BMS-ALM-2405", time: "10:28:31", priority: "Medium", asset: "GBIA-CH-PLANT-01", message: "Condenser approach temperature rising", state: "Investigating", owner: "Utility Plant" },
  { id: "BMS-ALM-2404", time: "10:15:12", priority: "Low", asset: "GBIA-FCU-T1-L2-18", message: "Filter runtime reaches 86%", state: "Planned", owner: "Terminal FM" },
  { id: "BMS-ALM-2403", time: "09:58:40", priority: "Info", asset: "GBIA-CRAH-DC-03", message: "Redundancy test completed", state: "Closed", owner: "Data Center" },
];
...
function BmsOverview({ onOpenEquipment, selected }: { onOpenEquipment: (id: string) => void; selected: BMSEquipment }) {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
      ...
      <AirportPanel title={vi ? "Sự kiện vận hành trực tiếp" : "Live operational events"}><div className="p-3"><AirportTimeline events={[["10:42","AHU","CO₂ threshold exceeded","T1-L2","Warning"],["10:36","VAV","Airflow deviation detected","T1-L3","Warning"],["10:28","Chiller","Efficiency recalculated","Plant","Normal"],["10:15","FCU","Maintenance runtime updated","Retail","Info"]]} /></div></AirportPanel>
    </div>
  </>;
}
...
function EquipmentLibrary({ selected, onSelect }: { selected: BMSEquipment; onSelect: (id: string) => void }) {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3">
    ...
    <div className="grid min-h-0 flex-1 items-stretch gap-3 xl:grid-cols-[300px_minmax(0,1fr)] 2xl:grid-cols-[330px_minmax(0,1fr)]">
      <AirportPanel title={vi ? "Thư viện thiết bị BMS" : "BMS equipment library"} subtitle={vi ? "Toàn bộ model được kế thừa từ module Viettel Đà Nẵng" : "Models inherited from the previous Viettel Da Nang BMS module"} className="h-full">
...
function TerminalZoneOperations({ selected, onSelect }: { selected: BMSEquipment; onSelect: (id: string) => void }) {
  ...
  return <>
    <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[[vi?"Nhiệt độ trung bình":"Average temperature","23.4°C","Comfort"],[vi?"Độ ẩm":"Humidity","54%","Healthy"],["CO₂","612 ppm","Good"],[vi?"Zone đang phục vụ":"Served zones","148","T1"],[vi?"Thiết bị hoạt động":"Active equipment","382","98.4%"],[vi?"Zone cảnh báo":"Zones in warning","4","Review"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===5?"amber":"cyan"}/>)}</div>
    <div className="grid gap-4 xl:grid-cols-[.75fr_1.25fr]">
      <AirportPanel title={vi?"Bản đồ zone tiện nghi":"Terminal comfort zones"}>
